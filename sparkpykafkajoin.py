...
spark = SparkSession.builder.appName('redis-app').getOrCreate()
...
redisStreamDF = redisRawStreamDF.selectExpr('cast(value as string) value')
...
redisStreamDF.withColumn('value', from_json('value', redisMessageSchema)) \
    .select(col('value.*')) \
...
redisSelectView = spark.sql('select key, zSetEntries[0].element as encodedCustomer from RedisSortedSet')
...
redisDecodeDF = redisSelectView.withColumn('encodedCustomer', unbase64(redisSelectView.encodedCustomer).cast('string'))
# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
redisDecodeDF.withColumn('encodedCustomer', from_json('encodedCustomer', customerJSONSchema)) \
...
stediRawStreamDF = spark \
    ...
    .load()
# TO-DO: cast the value column in the streaming dataframe as a STRING
steditreamDF = stediRawStreamDF.selectExpr('cast(value as string) value')
...
steditreamDF.withColumn('value', from_json('value', stediJSONSchema)) \
    .select(col('value.*')) \
    .createOrReplaceTempView('CustomerRisk')
# TO-DO: execute a sql statement against a temporary view, selecting the customer and the score from the temporary view, creating a dataframe called customerRiskStreamingDF
customerRiskStreamingDF = spark.sql('select customerName, score from CustomerRisk')
# TO-DO: join the streaming dataframes on the email address to get the risk score and the birth year in the same dataframe
customerRiskAndBirthYearDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr("""customerName = encodedCustomer"""))
